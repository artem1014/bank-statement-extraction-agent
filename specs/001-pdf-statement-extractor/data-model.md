...
A canonical JSON Schema (Draft 2020-12) generated from the Zod schema is committed to
`specs/001-pdf-statement-extractor/contracts/extract-result.schema.json` and is the
input passed to Anthropic's tool-use as `tools[0].input_schema`. The JSON Schema is
regenerated by `pnpm -F @app/contracts run gen-schema` (uses `zod-to-json-schema`) and
must be in sync with `schemas.ts` — CI fails if `git diff` shows a delta after the
script runs.
